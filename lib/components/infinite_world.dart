...
class InfiniteWorld extends Component {
  ...
  void update(double dt) {
    ...
    for (
      int x = playerChunkX - loadDistance;
      x <= playerChunkX + loadDistance;
      x++
    ) {
      for (
        int y = playerChunkY - loadDistance;
        y <= playerChunkY + loadDistance;
        y++
      ) {
        final key = '$x,$y';
  ...
  void _loadChunk(int x, int y) {
    final key = '$x,$y';
    final chunk = WorldChunk(chunkX: x, chunkY: y, seed: seed);
...
class WorldChunk extends PositionComponent {
  ...
  WorldChunk({required this.chunkX, required this.chunkY, required this.seed}) {
...
class _Tile {
  ...
  _Tile({required this.position, required this.size, required this.type}) {
    paint = Paint()
      ..color = _getColor()
  ...
  void render(Canvas canvas) {
    canvas.drawRect(Rect.fromLTWH(position.x, position.y, size, size), paint);
  }
}
