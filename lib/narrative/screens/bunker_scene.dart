...
class _BunkerSceneState extends State<BunkerScene>
  ...
  Future<void> _loadWallTextures() async {
    try {
      final dataH = await rootBundle.load(
        'assets/images/wall_panel_horizontal.png',
      );
      ...
      final dataV = await rootBundle.load(
        'assets/images/wall_panel_vertical.png',
      );
  ...
  void _transitionToCombat() async {
    ...
    Navigator.of(context).pushReplacement(
      MaterialPageRoute(
        builder: (context) => const MyApp(startInBossMode: true),
      ),
    );
  }
  ...
  void _updatePlayerPosition() {
    ...
    if (velocity.x != 0 || velocity.y != 0) {
      ...
      // Intentar mover en X
      double nextX = (_playerPosition.x + velocity.x).clamp(
        _playerSize / 2,
        room.roomSize.width - _playerSize / 2,
      );
      ...
      // Intentar mover en Y
      double nextY = (_playerPosition.y + velocity.y).clamp(
        _playerSize / 2,
        room.roomSize.height - _playerSize / 2,
      );
    ...
    if (pos.x - halfSize < thickness) {
      ...
      for (final door in room.doors) {
        if (door.position.x <= 10) {
          ...
          if (pos.y >= door.position.y &&
              pos.y <= door.position.y + door.size.y) {
            inDoor = true;
            break;
          }
        }
    ...
    if (pos.y - halfSize < thickness) {
      ...
      for (final door in room.doors) {
        if (door.position.y <= 10) {
          if (pos.x >= door.position.x &&
              pos.x <= door.position.x + door.size.x) {
            inDoor = true;
            break;
          }
        }
  ...
  List<Widget> _buildRoomContent(RoomData room) {
    ...
    final trees = room.interactables
        ...
        .toList();
    final otherInteractables = room.interactables
        .where((i) => i.type != InteractableType.decoration)
        .toList();
    ...
    return [
      ...
      if (_treeSprite != null && trees.isNotEmpty)
        Positioned.fill(
          child: CustomPaint(
            painter: ForestPainter(image: _treeSprite!, trees: trees),
          ),
        ),

      // Paredes (WallWidgets)
      if (_wallHorizontal != null &&
          _wallVertical != null &&
          room.id != 'exterior_large')
        ..._buildWalls(room),
  ...
  List<Widget> _buildWalls(RoomData room) {
    ...
    // Top Wall
    final topDoors = room.doors
        .where((d) => d.position.y <= 10)
        .toList(); // Tolerancia
    if (topDoors.isEmpty) {
      walls.add(
        Positioned(
      ...
      if (currentX < room.roomSize.width) {
        walls.add(
          Positioned(
            ...
            left: currentX,
            child: WallWidget(
              width: room.roomSize.width - currentX,
              height: thickness,
              side: WallSide.top,
              image: _wallHorizontal,
            ),
    ...
    // Bottom Wall
    final bottomDoors = room.doors
        .where((d) => d.position.y >= room.roomSize.height - thickness - 10)
        .toList();
      ...
      if (currentY < room.roomSize.height) {
        walls.add(
          Positioned(
            ...
            child: WallWidget(
              width: thickness,
              height: room.roomSize.height - currentY,
              side: WallSide.left,
              image: _wallVertical,
            ),
          ),
    ...
    // Right Wall
    final rightDoors = room.doors
        .where((d) => d.position.x >= room.roomSize.width - thickness - 10)
        .toList();
