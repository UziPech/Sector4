...
class BunkerBossLevel extends Component
  ...
  Future<void> onLoad() async {
    ...
    for (int i = 0; i < objectPositions.length; i++) {
      if (i == realObjectIndex) {
        ...
        // Este es un DECOY
        final decoy = DestructibleObject(position: objectPositions[i]);
    ...
    final tutorialSequence = DialogueSequence(
      ...
      dialogues: [
        ...
        const DialogueData(
          speakerName: 'Mel',
          text:
              'Escucha, Dan. Es un Resonante. Es invulnerable mientras su "Objeto Obsesivo" exista.',
  ...
  void _createBunkerLayout() {
    ...
    _addFloor(
      ...
      textureFile: 'metal_floor_clean.png',
    );
    ...
    _addRoomWalls(
      Vector2(0, 500),
      Vector2(700, 500),
      doors: [DoorPos.top, DoorPos.left, DoorPos.right, DoorPos.bottom],
    );
    _addFloor(
      Vector2(0, 500),
      Vector2(700, 500),
    ...
    _addRoomWalls(
      Vector2(-700, 500),
      Vector2(700, 500),
      doors: [DoorPos.right, DoorPos.top],
    );
    _addFloor(
      Vector2(-700, 500),
      Vector2(700, 500),
    ...
    _addRoomWalls(
      Vector2(700, 500),
      Vector2(700, 500),
      doors: [DoorPos.left, DoorPos.right],
    );
    _addFloor(
      Vector2(700, 500),
      Vector2(700, 500),
    ...
    _addRoomWalls(
      Vector2(0, 1000),
      Vector2(700, 500),
      doors: [DoorPos.top, DoorPos.bottom],
    );
    _addFloor(
      Vector2(0, 1000),
      Vector2(700, 500),
      const Color(0xFF3A3A4A),
      textureFile: 'metal_floor_quarters.png',
    );
  ...
  void _addRoomWalls(
    ...
    if (doors.contains(DoorPos.top)) {
      game.world.add(
        Wall(
          pos,
          Vector2((size.x - doorSize) / 2, thickness),
          side: WallSide.top,
        ),
      ); // Left part
      game.world.add(
        Wall(
          pos + Vector2((size.x + doorSize) / 2, 0),
          Vector2((size.x - doorSize) / 2, thickness),
          side: WallSide.top,
        ),
      ); // Right part
    ...
    if (doors.contains(DoorPos.bottom)) {
      game.world.add(
        Wall(
          pos + Vector2(0, size.y - thickness),
          Vector2((size.x - doorSize) / 2, thickness),
      ...
      game.world.add(
        Wall(
          pos + Vector2((size.x + doorSize) / 2, size.y - thickness),
          Vector2((size.x - doorSize) / 2, thickness),
          side: WallSide.bottom,
        ),
      );
    ...
    if (doors.contains(DoorPos.left)) {
      game.world.add(
        Wall(
          pos,
          Vector2(thickness, (size.y - doorSize) / 2),
          side: WallSide.left,
        ),
      );
      game.world.add(
        Wall(
          pos + Vector2(0, (size.y + doorSize) / 2),
          Vector2(thickness, (size.y - doorSize) / 2),
          side: WallSide.left,
        ),
      );
      ...
      game.world.add(
        Wall(pos, Vector2(thickness, size.y), side: WallSide.left),
      );
    }
    ...
    if (doors.contains(DoorPos.right)) {
      game.world.add(
        Wall(
          pos + Vector2(size.x - thickness, 0),
          Vector2(thickness, (size.y - doorSize) / 2),
          side: WallSide.right,
        ),
      );
      game.world.add(
        Wall(
          pos + Vector2(size.x - thickness, (size.y + doorSize) / 2),
          Vector2(thickness, (size.y - doorSize) / 2),
          side: WallSide.right,
        ),
      );
  ...
  void _activateExitDoor() {
    ...
    debugPrint(
      'EXIT DOOR TRIGGER CREATED at position: ${exitTrigger.position}',
    );
  }
    ...
    final postResonanteSequence = DialogueSequence(
      ...
      dialogues: const [
        ...
        DialogueData(
          speakerName: 'Dan',
          text:
              'Aguantar. Siempre aguantar. Como si mi vida entera no hubiera sido eso... aguantar hasta que algo se rompa.',
          type: DialogueType.internal,
        ),
        DialogueData(
          speakerName: 'Mel',
        ...
        for (double x = 0; x < size.x; x += imgWidth) {
          ...
          canvas.drawImageRect(
            ...
            Rect.fromLTWH(
              ...
              w,
              srcRect.height,
            ), // Recorte ajustado al ancho restante
        ...
        for (double y = 0; y < size.y; y += imgHeight) {
          ...
          canvas.drawImageRect(
            ...
            Rect.fromLTWH(
              ...
              srcRect.width,
              h,
            ), // Recorte ajustado al alto restante
...
class RedAlertOverlay extends Component
    with HasGameReference<ExpedienteKorinGame> {
  double _timer = 0;
...
class BlackScreenOverlay extends Component
    with HasGameReference<ExpedienteKorinGame> {
  double opacity = 1.0;
...
class ExitTransitionOverlay extends Component
  ...
  void update(double dt) {
    ...
    if (_timer < 0.5) {
      debugPrint(
        'ExitTransitionOverlay updating: timer=$_timer, opacity=$opacity',
      );
    }
